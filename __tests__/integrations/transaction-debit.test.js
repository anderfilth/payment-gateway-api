...
describe('Transaction with debit card', () => {
  ...
  it('should try to successfully create a transaction with debit card', async () => {
    ...
    expect(getPayable.body).toHaveProperty('payment_status', 'paid');
    expect(getPayable.body).toHaveProperty(
      'payment_date',
      moment().format('YYYY-MM-DD')
    );
    expect(getPayable.body).toHaveProperty('amount', 9700);
    expect(getPayable.body).toHaveProperty('fee', 300);
  });
});
